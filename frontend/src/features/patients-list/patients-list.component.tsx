...
export function PatientsListComponent({
  ...
  const handleCalledChange = (index: number, value: boolean) => {
    setPatientsState((prev) =>
      prev.map((p, i) => (i === index ? {...p, called: value} : p))
    );
  };

  const handleAcceptedChange = (index: number, value: boolean) => {
    setPatientsState((prev) =>
      prev.map((p, i) => (i === index ? {...p, accepted: value} : p))
    );
  };
        ...
        <tbody className="bg-white">
          {patients.map((patient) => (
            <tr
              ...
              className="transition duration-200 ease-in-out not-last:border-b not-last:border-gray-200 hover:cursor-pointer hover:bg-sky-100"
            ...
            >
              {Object.keys(patient).map((header) => {
                ...
                return (
                  <td key={header} className="px-3 py-3.5 text-left break-all">
                    {value}
                  </td>
                );
              })}
